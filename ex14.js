...
function rollDice(numFaces) {
  ...
  const scaledNumber = randomNumber * (numFaces - 1);
  const result = Math.floor(scaledNumber) + 1;
  return result;
}

const numFaces = 6;
const diceRoll = rollDice(numFaces);
console.log("El resultado del lanzamiento del dado es la cara con el número:", diceRoll);
